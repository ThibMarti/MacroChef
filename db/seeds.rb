...
# Seeds the Recipe catalog from the original hardcoded dish list (formerly
# RecipeCatalog::RECIPES/BREAKFAST_OPTIONS/SNACK_OPTIONS). Each recipe starts
# with a single placeholder ingredient (its own name, "1 serving") carrying
# the dish's full macros, so totals match exactly on day one — from the
# Recipes UI, users can then break that down into a real ingredient list.
MAIN_DISHES = [
...
def seed_recipes(dishes, category)
  dishes.each do |dish|
    recipe = Recipe.find_or_create_by!(name: dish[:name]) { |r| r.category = category }
...
seed_recipes(MAIN_DISHES, "main")
seed_recipes(BREAKFAST_DISHES, "breakfast")
seed_recipes(SNACK_DISHES, "snack")

puts "Seeded #{Recipe.count} recipes (#{Recipe.where(category: 'main').count} main, " \
     "#{Recipe.where(category: 'breakfast').count} breakfast, #{Recipe.where(category: 'snack').count} snack)."
